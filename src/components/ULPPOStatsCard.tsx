...
export const ULPPOStatsCard: React.FC<ULPPOStatsCardProps> = ({ ulpData, onDetailClick, ulpList }) => {
  ...
  const sortedData = requestedOrder.map(name => {
    ...
    const found = ulpData.find(d => d.ulp.replace(/\s+/g, "").toUpperCase().includes(normName));
    return {
      ulp: name, // Always display the pristine capitalized version with correct spaces
      jumlahPoTotal: found ? found.jumlahPoTotal : 0,
      totalPoPakaiCctv: found ? found.totalPoPakaiCctv : 0,
      persenPo: found ? found.persenPo : "0%"
    };
  });
